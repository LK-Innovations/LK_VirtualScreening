#!/bin/bash
#
# Standalone script for Snakemake rule: run_graphdta
# Submits SLURM jobs for GraphDTA initial screening (one per input chunk).
#
...
EXE="/home/ubuntu/screening_workflow/algos/GraphDTA/predict.py"
MODEL="/home/ubuntu/screening_workflow/algos/GraphDTA/model_para/model_GINConvNet_kiba.pt"
CONDA_ENV="graphdta"
...
submit_job() {
    ...
    local OUTPUT_DIR="${INIT_DIR}/GraphDTA"
    ...
    cat > "$JOB_SCRIPT" <<EOF
#!/bin/bash
#SBATCH --job-name=graphdta_${protein}_${idx}
...
log_info "Starting GraphDTA batch job submission"
...
for PROTEIN in $PROTEINS; do
    ...
    log_info "Submitted ${SUBMITTED} GraphDTA jobs for ${PROTEIN}"
done

log_info "All GraphDTA jobs submitted"
